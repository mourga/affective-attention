...
if one_exp:
    ...
    # yamls_base = []
    yamls_base_concat = ["baseline_concat_emb.yaml", "baseline_concat_rnn.yaml"]
    # yamls_base_concat = ["baseline_concat_rnn.yaml"]
    yamls = ["concat.yaml", "gating.yaml", "non_linear_affine.yaml", "affine.yaml"]
    ...
    for yaml in yamls_base_concat:
        ...
        acc_tests = []
        for _ in range(5):
            val_loss, acc, f1, precision, recall, f1_test, acc_test = clf_features_baseline_runner(os.path.join(yamls_path,
                                                                                    "{}".format(yaml)), word2idx, idx2word, weights, cluster=True)
            val_losses.append(val_loss)
            accs.append(acc)
            f1s.append(f1)
    ...
    path = os.path.join(BASE_DIR, "json_files")
    now = datetime.datetime.now().strftime("%y-%m-%d_%H:%M:%S")
    name = "{}_new_experiments_baseline_concat".format(yamls_path)
    name += "_{}".format(now)
    name += ".json"
    with open(os.path.join(path, name), 'w') as f:
        json.dump(experiments, f)
    ...
    for yaml in yamls:
        ...
        acc_tests = []
        for _ in range(5):
            val_loss, acc, f1, precision, recall, f1_test, acc_test = clf_features_runner(os.path.join(yamls_path,
                                                                                    "{}".format(yaml)), word2idx, idx2word, weights, cluster=True)
            val_losses.append(val_loss)
            accs.append(acc)
            f1s.append(f1)
    ...
    path = os.path.join(BASE_DIR, "json_files")
    now = datetime.datetime.now().strftime("%y-%m-%d_%H:%M:%S")
    name = "{}_new_experiments".format(yamls_path)
    name += "_{}".format(now)
    name += ".json"
    with open(os.path.join(path, name), 'w') as f:
        json.dump(experiments, f)
